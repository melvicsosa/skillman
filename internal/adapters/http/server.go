...
func NewHandler(svc *app.Service) (http.Handler, error) {
	...
	mux.HandleFunc("POST /api/vault/{name}/sync", s.handleSyncVault)
	mux.HandleFunc("PATCH /api/vault/{name}", s.handlePatchVault)
	mux.HandleFunc("POST /api/vault/adopt", s.handleAdoptVault)
	mux.HandleFunc("POST /api/vault/export", s.handleExportPlugin)
	mux.HandleFunc("POST /api/sync", s.handleSyncAll)
	mux.HandleFunc("POST /api/doctor/drift/{name}/repair", s.handleRepairDrift)
